 ...
 * types, Domain value types, and UseCases. The remaining rules (UseCases never call UseCases,
 * Operations never own transactions or call Operations, events emitted only by UseCases) are
...
if ($controllers !== []) {
    arch('Pulsar Controllers depend only on delivery types, Domain values, and UseCases')
        ->expect($controllers)
